...
import (
	"context"
	"fmt"
	"github.com/wahyudibo/golang-reverse-proxy/modules/ahrefs/internal/limiter"
	"net/http"
	"os"
	"os/signal"
	"syscall"
...
func main() {
	...
	err = db.Ping()
	if err != nil {
		log.Fatal().Err(err).Msg(fmt.Sprintf("host: %s, port: %d, user: %s, password: %s, database: %s",
			cfg.DBMySQLHost, cfg.DBMySQLPort, cfg.DBMySQLUser, cfg.DBMySQLPassword, cfg.DBMySQLDatabase))
	}
	...
	limiter.L = limiter.NewLimiter(repo)
	...
	r.Use(limiter.AccessLimitChecker)
